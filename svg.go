...
type SVG struct {
	*Element
	pretty bool // Pretty SVG layout
}

// New gives the user a new SVG
func New() (svg *SVG) {
	svg = &SVG{
		Element: newElement("svg", nil),
		pretty:  false,
	}
...
// NewWH gives the user a new SVG with the width and height set
func NewWH(w, h int) (svg *SVG) {
	return New().Width(w).Height(h)
}

// NewWHT gives the user a new SVG with the width, height & title set
func NewWHT(w, h int, title string) (svg *SVG) {
...
// Pretty determines if the output of the SVG is pretty or not
func (svg *SVG) Pretty(pretty bool) {
	if svg == nil {
		return
	}
	svg.pretty = pretty
...
// Marshal marshals the SVG into XML
func (svg *SVG) Marshal() ([]byte, error) {
	var b bytes.Buffer
	bufWriter := bufio.NewWriter(&b)
	if err := svg.Write(bufWriter); err != nil {
		return nil, err
	}
...
func (svg *SVG) Write(writer io.Writer) error {
	if svg == nil || len(svg.elements) == 0 {
		return fmt.Errorf("no svg data")
	}
	return writeElement(svg.Element, writer, 0, svg.pretty)
}

// Width is a helper that sets the width of the SVG
func (svg *SVG) Width(w int) *SVG {
	svg.Element.AttrInt("width", w)
	return svg
}
...
// Title is a helper that sets the title of the SVG
func (svg *SVG) Title(t string) *SVG {
	svg.Element.Attr("title", t)
	return svg
}

func (svg *SVG) Class(class string) *SVG {
	svg.class = class
	return svg
}

func (svg *SVG) Style(k, v string) *SVG {
	svg.Element.Style(k, v)
	return svg
}

func (svg *SVG) Transform(t Transformation) *SVG {
	svg.Element.Transform(t)
	return svg
}
