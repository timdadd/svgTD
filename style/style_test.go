...
const (
	expectedPrettySD = `<svg viewBox="0 0 10 10" xmlns="http://www.w3.org/2000/svg">
  <style>
    circle {
      fill: gold;
      stroke: maroon;
...
// TestStyle is a GO Implementation of https://developer.mozilla.org/en-US/docs/Web/SVG/Element/style
func TestStyle(t *testing.T) {
	// Create the s canvas
	var s = svgTD.New().ViewBox(0, 0, 10, 10)
	s.Element.Append("style").Append("circle").
		Attr("fill", "gold").
		Attr("stroke", "maroon").
		Attr("stroke-width", "2px")
	s.Circle().Attrs(5, 5, 4)
	s.Pretty(true)
